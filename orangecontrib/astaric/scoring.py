...
def GMM(X, k):
    gmm = sklearn.mixture.GaussianMixture(n_components=k, covariance_type='diag')
    gmm.fit(X)
    return Result(gmm.weights_, gmm.means_, gmm.covariances_, k, [])
...
def get_cluster_weights(priors, means, covars, x, crisp=True, eps=1e-15):
    ...
    w = np.zeros((len(x), k))
    for j in range(k):
        if np.any(np.abs(covars[j]) < 1e-15):
            continue
